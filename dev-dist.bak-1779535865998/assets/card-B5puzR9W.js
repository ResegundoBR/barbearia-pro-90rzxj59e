import { a as e, n as t, t as n } from './jsx-runtime-m7G7yzlP.js'
import { J as r } from './index-CXmOAr77.js'
var i = e(t(), 1),
  a = n(),
  o = i.forwardRef(({ className: e, ...t }, n) =>
    (0, a.jsx)(`div`, {
      ref: n,
      className: r(`rounded-lg border bg-card text-card-foreground shadow-sm`, e),
      ...t,
    }),
  )
o.displayName = `Card`
var s = i.forwardRef(({ className: e, ...t }, n) =>
  (0, a.jsx)(`div`, { ref: n, className: r(`flex flex-col space-y-1.5 p-6`, e), ...t }),
)
s.displayName = `CardHeader`
var c = i.forwardRef(({ className: e, ...t }, n) =>
  (0, a.jsx)(`div`, {
    ref: n,
    className: r(`text-2xl font-semibold leading-none tracking-tight`, e),
    ...t,
  }),
)
c.displayName = `CardTitle`
var l = i.forwardRef(({ className: e, ...t }, n) =>
  (0, a.jsx)(`div`, { ref: n, className: r(`text-sm text-muted-foreground`, e), ...t }),
)
l.displayName = `CardDescription`
var u = i.forwardRef(({ className: e, ...t }, n) =>
  (0, a.jsx)(`div`, { ref: n, className: r(`p-6 pt-0`, e), ...t }),
)
u.displayName = `CardContent`
var d = i.forwardRef(({ className: e, ...t }, n) =>
  (0, a.jsx)(`div`, { ref: n, className: r(`flex items-center p-6 pt-0`, e), ...t }),
)
d.displayName = `CardFooter`
export { s as a, d as i, u as n, c as o, l as r, o as t }
